...
export default function VuotChuongNgaiVat() {
  ...
  const [startGame, setStartGame] = useState(false); // New state for controlling game start
  ...
  const handleCloseQuestion = () => {
    ...
    setShowJudge(false);
    setMode("normal");
    setStartGame(false); // Reset start game state
    if (audioBellRef.current) {
      audioBellRef.current.pause();
      audioBellRef.current.currentTime = 0;
    }
          ...
          {clues.map((row, rowIdx) => (
            <div key={rowIdx} className="flex gap-2 items-center mb-2">
              <button
                ...
                className={`w-12 h-12 rounded-full text-2xl font-bold flex-shrink-0
                  border-4 text-white
                  ${statuses[rowIdx] === "correct"
                ...
                return (
                  <div
                    ...
                    className={`w-12 h-12 border-2 flex items-center justify-center
                      ...
                      ${status === "correct" ? "bg-purple-600 text-white" : status === "wrong" ? "bg-gray-400 text-gray-300" : "bg-white text-black"}
            ...
            >
              ...
              {!startGame && (
                <button
                  ...
                  className="bg-yellow-500 text-white px-10 py-4 rounded-lg font-bold text-3xl hover:bg-yellow-600"
                >
                  Bắt đầu
                </button>
              )}
